export const MovieRequests = {
  fetchTrending: `/trending/all/week?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`,
  fetchNetflixOriginals: `/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&with_networks=213`,
  fetchTopRated: `/movie/top_rated?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`,
  fetchActionMovies: `/discover/movie?api_key=${process.env.REACT_APP_API_KEY}&with_genres=28`,
  fetchComedyMovies: `/discover/movie?api_key=${process.env.REACT_APP_API_KEY}&with_genres=35`,
  fetchHorrorMovies: `/discover/movie?api_key=${process.env.REACT_APP_API_KEY}&with_genres=27`,
  fetchRomanceMovies: `/discover/movie?api_key=${process.env.REACT_APP_API_KEY}&with_genres=10749`,
  fetchDocumentaries: `/discover/movie?api_key=${process.env.REACT_APP_API_KEY}&with_genres=99`,
};

export const TvShowRequests = {
  fetchTrending: `/trending/tv/week?api_key=${process.env.REACT_APP_API_KEY}`,
  fetchTopRated: `/tv/top_rated?api_key=${process.env.REACT_APP_API_KEY}`,
  fetchComedies: `/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&with_genres=35`,
  fetchRomance: `/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&with_genres=10749`,
  fetchDocumentaries: `/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&with_genres=99`,
};

export const PopularRequests = {
  fetchNewTVShows: `/tv/on_the_air?api_key=${process.env.REACT_APP_API_KEY}`,
  fetchNewMovies: `/movie/now_playing?api_key=${process.env.REACT_APP_API_KEY}`,
  fetchPopularTvShows: `/tv/popular?api_key=${process.env.REACT_APP_API_KEY}`,
  fetchPopularMovies: `/movie/popular?api_key=${process.env.REACT_APP_API_KEY}`,
};
